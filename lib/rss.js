...
export default async function generateRssFeed() {
  ...
  const feed = new Feed({
    title: `Webkoof.in | Code with Bharat Sahu`,
    description: "Code with Bharat Sahu! JavaScript, React, Next.js, Node.js and Firebase tutorials.",
    id: baseUrl,
    link: baseUrl,
    language: "en",
  ...
  await Promise.all(posts.map((post) => new Promise((resolve, reject) => {
    getPostData(post.id)
        .then(data => {
          feed.addItem({
            title: post.title,
            id: `${baseUrl}/articles/${data.id}`,
            link: `${baseUrl}/articles/${data.id}`,
